...
ggplot(data.frame(Date = as.Date(wind_data$DATE),as.numeric(rep_trend))) +
  geom_line(aes(x=Date, y=rep_trend, color="Decompose"))+
  labs(color="")+
  scale_color_manual(values=c("Repeoduced"="blue", "Decomposed" = "red"), labels=c("Manually calculated", "From decompose function"))+
...
orig_minus_trend <- data.frame(Date = as.Date(wind_data$DATE),as.numeric(orig_minus_trend))
...
ggplot(orig_minus_trend_by_month,aes(x=month))+geom_line(aes(y=rep_seas, color="Reproduced"))+
